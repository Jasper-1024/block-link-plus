...
Obsidian 原生前进／返回支持 Outliner 文件切换、块链接跳转和 Zoom。返回时恢复原来的聚焦范围、折叠状态与滚动位置；顶部面包屑仍用于返回祖先层级，不等同于访问历史。
...
- `Alt+↑` / `Alt+↓`：移动当前 block 及完整子树。默认只在直接同级块之间移动；可在设置中选择“跨层级对齐”，以相邻可见块为目标并采用其层级。两种模式都会尊重当前可见范围，包括折叠与 Zoom 边界。
