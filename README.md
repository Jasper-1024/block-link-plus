...
[![Version](https://img.shields.io/badge/version-2.0.19-blue.svg)](https://github.com/Jasper-1024/obsidian-block-link-plus/releases)
...
### 2.0.19
- Reverted: withdrew the Live Preview embedded-search feature and Outliner wiki-link suggestion dismissal introduced in 2.0.18 because of reported regressions; their task branches remain available for correction

### 2.0.18
- Added: current-note search in Live Preview now includes BLP-managed inline block, heading, and file embeds, with native-panel navigation and visible-range matching
- Fixed: embedded-search results and counts now refresh after trusted source synchronization and inline-edit setting/lifecycle cleanup
- Fixed: stale Outliner wiki-link suggestions dismiss when their trigger is edited or removed
...
- Fix: inline embed editing avoids passive Live Preview scroll jumps, preserves the jump affordance, and removes extra bottom padding
- Fix: inline-edit undo/redo keeps embedded editor ranges bounded so nearby note content does not leak into embeds
- Fix: Outliner source-line navigation/search now resolves owning blocks more reliably
