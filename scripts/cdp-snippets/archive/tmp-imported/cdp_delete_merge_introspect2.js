(async () => {
  ...
  const tmpPath = `${tmpFolder}/delete-merge-debug2.md`;
  ...
  // Find the next sibling using the engine helper behavior.
  const blocks = view.outlinerFile?.blocks ?? [];
  const idx = blocks.findIndex((b) => b.id === editingId);
  const nextText = idx >= 0 ? blocks[idx + 1]?.text ?? null : null;

  return { ok: true, before, editingId, sel, editorValue: editor.value, nextText };
})();
