...
相關設定在：設定 → Block Link Plus → `增強清單` Tab（或用頂部搜尋）。
...
- `增強清單` Tab →「增強 List Blocks」→「顯示列表把手」
...
- `增強清單` Tab →「增強 List Blocks」→「啟用列表把手動作」
...
可在設定中調整左鍵行為：
- `增強清單` Tab →「增強 List Blocks」→「把手左鍵動作」

當「把手左鍵動作」選擇 `select-block` 時（僅啟用檔案 + Live Preview）：
- 左鍵點擊把手：選取該清單項作為一個區塊（區塊高亮，不是文字選取）。
- Shift + 左鍵：連續多區塊選取（以區塊為粒度）。
- Esc：清除區塊選取並回到一般編輯。

### 巢狀程式碼區塊縮進（可選）

預設開啟（可在設定中關閉）：
- `增強清單` Tab →「增強 List Blocks」→「縮進巢狀程式碼區塊」

啟用後，清單項內的圍欄程式碼區塊會在 Live Preview 中依層級縮進顯示（僅渲染，不修改檔案內容）。

### 目前區塊高亮（內建）

在 Live Preview 中，當游標位於某個清單項（包含續行/程式碼區塊/系統行）內時，外掛會以背景高亮整個 list block，方便識別目前區塊邊界。
...
- `增強清單` Tab →「增強 List Blocks」→「刪除列表項時刪除子項」
...
在 `增強清單` Tab 的「blp-view（Query/View）」分組中，可設定 `blp-view` 相關護欄（僅在 Dataview 可用時顯示）：
...
- `內建外掛` Tab →「內建外掛（vslinko）」→「將內建列表互動限定到 Enhanced List」
