...
- [x] `doc/api.md` / `doc/en/api.md` / `doc/zh-TW/api.md`：无 TODO 占位；`plugin.api` 说明与 `src/main.ts` 一致
- [x] `doc/reference/commands.md`（三语）：Outliner 命令 ID 与默认快捷键描述正确
- [x] `doc/reference/settings.md`（三语）：settings key 与默认值与 `src/types/index.ts` 一致
- [x] `doc/reference/yaml.md`（三语）：只写 `blp-view`，不再写 `blp-timeline`
- [x] `doc/usage/outliner.md`（三语）：说明“强 opt-in + 可能写回/规范化”的风险提示清晰
- [x] `doc/changelog.md`（三语）：2.0 置顶内容准确；不要把“未发布/开发中”写给用户（如需可留空的 Unreleased 标题也 OK）
...
- [x] `archive/openspec/changes/` 无未归档 change（目前应只有 `archive/`）
- [x] 如有需要：跑一次 `archive/openspec/` historical review; no active OpenSpec validate gate（CI 前先挡掉格式问题）
- [x] 对内文档（`docs/**`、`archive/memory-bank/**`）把“已废弃能力”（timeline/time section/vendored vslinko）标注为历史，避免误导后续开发

## 9) 发布动作（执行清单）

- [x] 合并：`2.0-outliner` → `master`（处理冲突）
- [ ] 打 tag：`2.0.0`
- [ ] 观察 GitHub Actions：
  - docs 部署（gh-pages）
